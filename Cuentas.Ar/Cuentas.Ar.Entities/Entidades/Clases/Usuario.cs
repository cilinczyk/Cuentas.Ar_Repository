...
namespace Cuentas.Ar.Entities
{
    ...
    public partial class Usuario
    {
        ...
        public decimal CapacidadAhorroPesos
        {
            get
            {
                return Registro?.Where(x => x.idTipoRegistro == eTipoRegistro.Ingreso && x.idCategoria == eCategoria.Ahorros && x.Fecha >= DateTime.Now.AddMonths(-11) && x.idMoneda == eMoneda.Pesos)?.Sum(x => x.Importe) / 12 ?? 0;
            }
        }
        public decimal CapacidadAhorroDolares
        {
            get
            {
                return Registro?.Where(x => x.idTipoRegistro == eTipoRegistro.Ingreso && x.idCategoria == eCategoria.Ahorros && x.Fecha >= DateTime.Now.AddMonths(-11) && x.idMoneda == eMoneda.Dolares)?.Sum(x => x.Importe) / 12 ?? 0;
            }
        }
    }
